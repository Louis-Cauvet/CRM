...
std::list<contact> gestionListeContacts::getListeC() const {return listeC;}
unsigned int gestionListeContacts::getNbTotal() const {return nbTotal;}
date::year_month_day gestionListeContacts::getDateDerniereSuppression() const {return dateDerniereSuppression;}


void gestionListeContacts::setNbTotal(const unsigned int &nb){
    nbTotal = nb;
}
void gestionListeContacts::setDateDerniereSuppression(const date::year_month_day &d){
 ...
 * @warning si les noms et les prénoms sont identiques, le premier contact est considéré comme le premier par ordre alphabétique
 */
 ...
 */
void gestionListeContacts::ajouterContact(const contact &c){
    listeC.push_back(c);
 ...
 * @warning une fois le contact supprimé, on enlève 1 au nombre total de contacts de la liste
 * @warning la date de dernière suppression d'un contact est mise à jour avec la date du jour
 */
void gestionListeContacts::supprimerContact(const contact &co){
    listeC.remove(co);
    this->nbTotal = nbTotal-1;
 ...
 */
void gestionListeContacts::modifierContact(const contact &c , const std::string &n , const std::string &p , const std::string &e , const std::string &m , const std::string &t , const std::string &ph){
 ...
 */
std::list<contact> gestionListeContacts::chercherCParNom(std::string &recherche){
    if(recherche == ""){
        return listeC;
    } else {
 ...
 */
std::list<contact> gestionListeContacts::chercherCParEntreprise(std::string &recherche){
    if(recherche == ""){
        return listeC;
    } else {
 ...
 * @return une liste de contacts contenant uniquement les contacts qui remplissent la condition
 */
std::list<contact> gestionListeContacts::chercherCParIntervalleDate(const date::year_month_day &d1, const date::year_month_day &d2){
